...
2.「拡張機能の再起動」ボタンが表示されるので、クリックする。  
3.Python拡張機能がグレーアウトしたことを確認したら再度右クリックし、「有効にする(ワークスペース)」をクリックする。  
4.ターミナルを起動させたままにしていた場合、「powershell」ボタンにマウスオーバーすると「次の拡張機能がこのターミナルの環境に変更を加えるため、ターミナルの再起動を要求しています。」と表示される。  
5.指示通り、「ゴミ箱」ボタンでターミナルを終了後、「Ctrl + @」のショートカットキーを押下して、ターミナルを再起動させる。  
:::
